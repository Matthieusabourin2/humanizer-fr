...
Over-triggering is a defect, not a safe default: a skill that fires on everything gets ignored, and a rewrite nobody asked for launders a voice. Do not fire on code or configuration, on literal translation, on text quoted or cited from another author, on prose that already reads as a specific person (see the preservation list below), or on samples under 40 words where there is not enough signal to judge. When a request is ambiguous, say what you would change and let the user confirm before rewriting.
...
Auto-loads `humanizer-context.md` from the project root if present, or the copy bundled next to SKILL.md. Use that file for brand samples and banned phrases.

## Hard constraint: no invented facts

The rewrite must not contain any fact, name, number, date, quote, price, metric, or citation that is not in the source text or supplied by the user. This outranks every craft technique in this skill.

Two techniques below actively push against it and are therefore **gated**. The Concretizer pass (Step 3) may only replace an abstraction with a specific that already exists in the source; if the source says "improves performance" and gives no number, the honest rewrite says what improved, not "cuts p99 from 900ms to 40ms". Soul Injection technique 3, sensory and experiential detail, may add stance, reaction, and rhythm, never a scene that did not happen: no invented 2am debugging session, no coffee that was never drunk.

Opinions, uncertainty, and reactions are voice and may be added where PERSONALITY applies. Facts may not. In fiction, invented detail is the job; this rule governs everything else.

In Step 5, ask a second question alongside "what still reads as AI": **"does the rewrite state any fact, name, number, date, price, or citation absent from the source?"** Answer it explicitly. A fabrication is a defect even when it sounds more human than the vague original. On client-facing text, a proposal, a quote, a case study, this is the only check that matters more than the score.
...
**Command routing first.** If `$ARGUMENTS` begins with a single-dash token (`-help`, `-empreinte`, `-voix`, `-score`, `-fr`, `-audit`), read [`references/commands.md`](references/commands.md), execute that command, and stop. Single dash is a command, double dash is an option. Anything else falls through to the normal parsing below.
...
**Auto-load brand context.** Before parsing further, check for `humanizer-context.md` in the current working directory using the Read tool, and also next to this SKILL.md (bundled profile shipped with the skill). Load whichever exists as additional voice guidance (brand samples, banned phrases, preferred terms), a personal extension of the `--voice` profile. If both exist, the working-directory file takes precedence for same-named voices. If neither exists, proceed without warning; this is opt-in.
...
**You cannot compute this from inside the model.** Do not report a burstiness figure you did not measure. Either run the `cli/` tool, which computes coefficient of variation and the Goh-Barabasi index deterministically, or count by hand on 30 consecutive sentences and show the lengths so the reader can check. The observable proxy, when neither is available: count how many runs of 3+ consecutive sentences fall within 5 words of each other. Zero such runs is the target.
...
AI detectors also measure "perplexity": how predictable each word is. AI text has LOW perplexity, human text higher.

**You have no access to logprobs, so you cannot measure perplexity.** Never report a perplexity number. The observable proxies are type-token ratio, MATTR and trigram repetition, all computed by the `cli/` tool. Inside the model, use them only as a direction to write toward, never as a figure to quote.
...
Beyond the five built-ins, `--voice <name>` also accepts a custom profile defined in `humanizer-context.md`. Build one from your own writing with `/humanizer -empreinte`, driven by [`references/empreinte.md`](references/empreinte.md): it collects a corpus, measures it, and emits evidence-backed imitation rules plus the list of signature traits this skill must not scrub. A profile built from a real corpus outranks the built-in voices and the style rules here, em dash ban included.
...
**Anchor samples.** Calibrate against these, measured with the deterministic CLI, so the number means the same thing across sessions. A French paragraph loaded with tells (participle chains, *incontournable*, *force est de constater*, adjectival triad, *en conclusion*) scores **54**, with 8 lexical tells and burstiness 0.33. A plain human French narrative of the same length scores **0**, with 0 tells and burstiness 0.66. Three worked French examples with their before and after scores are in [`references/examples.fr.md`](references/examples.fr.md). If your figure for a comparable text lands far from these anchors, your count is wrong, not the text.

**Under 40 words, do not score.** Say the sample is too short. A short empty text scores near zero and is still worthless: the score measures AI tells, not quality.
