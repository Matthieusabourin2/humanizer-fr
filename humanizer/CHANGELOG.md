...
## 3.0.0-fr — 13 août 2026

Origine : trois défauts constatés à l'usage réel et confirmés par mesure — lenteur (94 à 165 s et 9 à 15 k tokens de sortie pour un simple score ou une réécriture d'un paragraphe, Sonnet 5, headless), scan non procédural qui s'arrête avant d'avoir couvert le catalogue, profil d'empreinte jamais appliqué comme contrainte à la réécriture. Analyse intégrale préalable de l'amont (46 commits, v0.5.0) et diff fichier par fichier : rien à backporter, mais trois leçons d'architecture à imiter (core mince toujours chargé, profondeur dans references/ à la demande, déterminisme délégué à du code).

**D22 — `scripts/scan.py` : scanner déterministe FR/EN (nouveau).** Portage Python Unicode du `cli/` amont, que le fork référençait sans le posséder (le tokenizer amont, ASCII-only, mutile « été » en « t »). Mêmes formules (burstiness CoV, Goh-Barabasi, TTR/MATTR fenêtre 50, répétition de trigrammes, score composite ancré) plus ce que l'amont n'a pas : le catalogue mécanique en regex (FR1-FR9, FR11, FR13, FR14 et 18 P-patterns par langue), les lexiques étagés FR, un mode `mesures` qui sort le tableau chiffré de la Passe 2 d'empreinte, `export-lexiques` pour réutiliser la source de vérité ailleurs (page de démo). Ancres FR recalibrées sur corpus réel le 13/08 : 12 textes humains scorent 0-19, les trois exemples « avant » 56-63, séparation minimale de 37 points ; le MATTR ne discriminant pas le français fléchi sur textes courts, le poids lexical passe à 0,55 en FR. Zéro dépendance, déterministe, < 0,1 s là où le modèle mettait 90 s.

**D23 — SKILL.md v3 : scan procédural en deux passes, corps 511 → 303 lignes.** La détection devient une procédure, pas une aspiration : passe mécanique d'abord (scan.py obligatoire quand l'exécution de code existe, ses chiffres sont des faits qu'on ne recompte pas), passe de jugement ensuite sur une liste fermée de groupes que le modèle coche un à un, fusion des deux AVANT toute réécriture. Sans exécution de code, passe catalogue manuelle avec comptes montrés. Le catalogue compact reste inline (autonomie du fallback), les développements longs restent dans references/. Corrections au passage : routage FR P17/P26 → P16/P47 (bug factuel), renvois au `cli/` inexistant remplacés par scan.py, Bash ajouté aux allowed-tools (requis pour les scripts ; le réseau reste banni).

**D24 — Voice Calibration + check 6 outillé (`SKILL.md`, `commands.md`, `scripts/gate.py`).** La section « Voice Calibration » que commands.md référençait n'existait nulle part — c'était la procédure d'application du profil qui manquait, cause directe du problème « empreinte ignorée ». Elle existe : relire le profil au moment de réécrire, appliquer chaque [R-nn] comme fourchette min-max, Interdits absolus, surface stricte, protection signature étendue à P54. Et elle se vérifie : le profil embarque un bloc ` ```json gate` ` (budgets, interdits, plafonds FRÉQUENCE des signatures) que `gate.py --profile` applique déterministiquement — le check 6, dernier laissé à l'auto-notation, est outillé.

**D25 — `references/empreinte.md` v3 : fusion du prompt v1.0 de l'auteur et de la version 2.x, outillée.** Reprend du prompt v1.0 le contrat de corpus `<TEXTE>`, les conditions d'arrêt, R1-R6, les passes et le schéma de sortie ; de la 2.x la Phase 0, la Passe 6 (humanizer-context.md, « Ce que humanizer ne touche pas ») et le verdict utilisateur. Ajoute : anti-contamination mesurée (chaque texte du corpus scoré par scan.py avant analyse, exclu au-delà de 40), Passe 2 entièrement outillée (`scan.py mesures`), test de bouclage jugé à l'aveugle par une session qui n'a pas écrit le profil, émission du bloc gate à partir des mesures, procédure de re-calibration datée.

**D26 — Hygiène.** `verify.py` : chemin amont en dur remplacé par `HUMANIZER_UPSTREAM`, fallback sans PyYAML (le crash `meta` en dépendait), section [13] réécrite sur scan.py (discrimination slop/humain, tokenizer Unicode, déterminisme deux passes) ; 123 tests verts. `__pycache__` retiré du dépôt, `.gitignore` ajouté. `always-on-templates.md` : compte de patterns corrigé. `examples.fr.md` : les six scores annoncés, écrits à l'époque où aucun CLI n'existait, remplacés par les chiffres réellement mesurés (63/13, 56/0, 58/23).

Vérification : python3 verify.py — 123 tests verts. Benchmarks avant/après dans docs/benchmarks.md.
